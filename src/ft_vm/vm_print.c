...
int			vm_print_usage(char **av, int status)
{
	...
	ft_printf_fd(2, "       -dumpc nbr_cycles  : ");
	ft_printf_fd(2, "%*wC%w%*wo%w%*wl%w%*wo%w%*wr%wized -dump\n", 1, 2, 3, 6, 5);
	ft_printf_fd(2, "       -v N : ");
	...
	ft_printf_fd(2, "       -u : Decompile .cor back to .s\n");
	ft_printf_fd(2, "       -g : Printf  visualization\n");
	ft_printf_fd(2, "       -G : Ncurses visualization\n");
	return (status);
}
