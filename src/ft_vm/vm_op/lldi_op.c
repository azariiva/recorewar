...
/*   Updated: 2020/11/12 20:14:07 by blinnea          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_init_param_long(t_process *p, int i)
{
	...
	result = (p->op.params[i].type == IND_CODE) ?
		ldd_memory(p->offset + p->pc + p->op.params[i].value, 4)
		: result;
	return (result);
}
...
void		lldi_op(t_vm *vm, t_process *p)
{
	...
	p->reg[p->op.params[2].value] = ldd_memory(cursor, 4);
	p->carry = (p->reg[p->op.params[2].value] == 0) ? 1 : 0;
	op_include(vm, p);
}

static void	vm_lldi_print_2(t_process p, int param0, int param1)
{
...
void		vm_lldi_print(t_process p)
{
	...
	param0 = (p.op.params[0].type == IND_CODE) ?
		ldd_memory((p.offset + p.pc + p.op.params[0].value), 4)
		: param0;
	vm_lldi_print_2(p, param0, param1);
}
